...
export const handleList = async (ctx: ContextMessageUpdate) => { 
  ...
  const res = await db.query(`select url, title from subscriptions as s join feeds as f on s.feed = f.url where s."user" = $1 and active order by most_recent_item desc;`, [ctx.chat.id])
  if (res.rowCount == 0) {
